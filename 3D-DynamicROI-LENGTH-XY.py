                ...
                for box, obj_id, conf in zip(boxes, ids, confs):
                    ...
                    if obj_id in counted_ids:
                        ...
                        if len(valid_points_for_length) >= 2:
                            ...
                            # Geçerli 3D noktaların hem X hem Y farklarına bak
                            x_coords_3d = [p[0] for p in valid_points_for_length]
                            y_coords_3d = [p[1] for p in valid_points_for_length]

                            if x_coords_3d and y_coords_3d:
                                x_range = max(x_coords_3d) - min(x_coords_3d)
                                y_range = max(y_coords_3d) - min(y_coords_3d)

                                # En büyük ekseni uzunluk olarak al
                                current_fish_length_mm = max(x_range, y_range)
